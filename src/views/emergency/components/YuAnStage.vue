<template>
  <div class="part things">
    ...
    <div v-if="operateType !== 'look'" class="add_btn" @click="addItem">
      <a-icon type="plus-circle" style="margin-right: 5px" />新增应急阶段
    </div>
    <div class="content">
      <a-row v-for="(item, index) in stageData" :key="index">
        <a-col :span="5">
          <div class="col-panel" flex="dir:left cross:center">
            <i
              class="dot"
              :class="{
                blue: item.stageName == '消息阶段',
                orange: item.stageName == '警报阶段',
                red: item.stageName == '紧急警报阶段',
                green: item.stageName == '警报解除阶段',
                active: activeStage == item.stageName
              }"
            ></i>
        ...
        <a-col :span="9">
          <div class="col-panel" flex="dir:left">
            ...
            <a-textarea v-model="item.description" placeholder="请输入" :autosize="{ minRows: 2, maxRows: 2 }" />
          </div>
        </a-col>
        <a-col :span="4">
          <div class="col-panel" flex="dir:left cross:center">
        ...
        <a-col :span="1" v-if="operateType !== 'look'">
          <a-popconfirm v-if="stageData.length > 1" title="确定删除这个阶段吗？" @confirm="() => deleteItem(index)">
