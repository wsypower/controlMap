      ...
      <a-row>
        <div class="col-panel" flex="dir:left">
          ...
          <a-select
            ...
            @change="
              val => {
                changeSelect(val, 'leaderOne')
              }
            "
            placeholder="请选择"
            :disabled="operateType === 'look'"
          >
            <a-select-option v-for="(item, index) in totalPeople" :key="item.id" :value="item.name + '_' + item.id"
              >{{ item.name }} {{ item.tel }}</a-select-option
            >
          </a-select>
        </div>
      </a-row>
    ...
    <div class="content member">
      ...
      <div class="group-member-panel" v-for="(groupItem, index) in groupMember" :key="index">
        <div class="member-top" flex="dir:left cross:center main:justify">
          ...
          <div v-if="operateType!=='look'">
            <a-icon
              v-if="index === groupMember.length - 1"
              type="plus-circle"
              style="margin-right: 10px;"
              @click="addGroupItem"
            />
            <a-popconfirm title="确定删除这个组吗？" @confirm="() => deleteGroupItem(index)">
        ...
        <a-row>
          <div class="col-panel" flex="dir:left">
            ...
            <a-select
              ...
              @change="
                val => {
                  changeSelect(val, index + '_groupForOne')
                }
              "
              placeholder="请选择"
              :disabled="operateType === 'look'"
        ...
        <a-row>
          <div class="col-panel" flex="dir:left">
            ...
            <a-select
              ...
              @change="
                val => {
                  changeSelect(val, index + '_groupForTwo')
                }
              "
              placeholder="请选择"
              :disabled="operateType === 'look'"
        ...
        <a-row>
          <div class="col-panel" flex="dir:left">
            ...
            <a-select
              ...
              @change="
                val => {
                  changeSelect(val, index + '_groupForThree')
                }
              "
              placeholder="请选择"
              :disabled="operateType === 'look'"
