<template>
  <div class="yuan-form-panel" flex="dir:top">
    <div class="liucheng-panel" flex="dir:left cross:center main:center">
      <span class="jieduan blue" :class="{ active: showStage == '消息阶段' }" @click="sendMessage('消息阶段')"
        >消息阶段</span
      >
      <cg-icon-svg name="jiantou" class="svg_icon_jiantou"></cg-icon-svg>
      <span class="jieduan orange" :class="{ active: showStage == '警报阶段' }" @click="sendMessage('警报阶段')"
        >警报阶段</span
      >
      <cg-icon-svg name="jiantou" class="svg_icon_jiantou"></cg-icon-svg>
      <span class="jieduan red" :class="{ active: showStage == '紧急警报阶段' }" @click="sendMessage('紧急警报阶段')"
        >紧急警报阶段</span
      >
      <cg-icon-svg name="jiantou" class="svg_icon_jiantou"></cg-icon-svg>
      <span class="jieduan green" :class="{ active: showStage == '警报解除阶段' }" @click="sendMessage('警报解除阶段')"
        >警报解除阶段</span
      >
    </div>
    <div class="yuan-form-panel-body">
      <cg-container scroll>
        <div class="yuan-form">
            ...
            <div class="content base">
              <a-row>
                <a-col :span="7">
                  ...
                  <div flex="dir:left">
                    ...
                    <span class="sub-content">
                      <span v-if="eventInfo.startDay">{{
                        new Date(parseInt(eventInfo.startDay)) | date_format('YYYY-MM-DD HH:mm')
                      }}</span>
                      <span v-if="eventInfo.endDay"
                        >~{{ new Date(parseInt(eventInfo.endDay)) | date_format('YYYY-MM-DD HH:mm') }}</span
                      >
                    </span>
                  </div>
                </a-col>
                <a-col :span="11" style="padding-right:20px;">
                  <div flex="dir:left">
                    <span class="sub-title">位置：</span><span class="sub-content">{{ eventInfo.position }}</span>
                  </div>
                  <div flex="dir:left">
                    <span class="sub-title">信息：</span><span class="sub-content">{{ eventInfo.description }}</span>
                  </div>
                  <div flex="dir:left">
                  ...
                  <div flex="dir:left">
                    ...
                    <span
                      v-for="(file, index) in JSON.parse(eventInfo.fileStr)"
                      :key="index"
                      class="file"
                      :title="file.oldName"
                      @click="downloadFile(file.newPath)"
                      >{{ file.oldName }}</span
                    >
                  </div>
                </a-col>
          ...
          <yu-an-people
            :peopleData.sync="peopleData"
            :operateType="operateType"
            @getResult="getPeopleResult"
          ></yu-an-people>
          <yu-an-resource
            :resourceData.sync="resourceData"
            :operateType="operateType"
    ...
    <div class="operate-panel" flex="main:center cross:center" v-if="sourceData.activeStage !== '警报解除阶段'">
      <a-button type="primary" @click="startYuAnEvent">{{
        sourceData.sourceType === 0 ? '启动预案' : '发送'
      }}</a-button>
    </div>
      ...
      .part {
        ...
        .content {
          ...
          span.file {
            ...
            &:hover {
              background-color: rgba(43, 144, 243, 0.6);
              color: #ffffff;
            }
          }
        }
