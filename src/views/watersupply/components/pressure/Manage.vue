    ...
    <div class="yuan_dialog_body">
      ...
      <cg-container scroll v-if="!showLoading && treeData.length > 0">
        <div class="result_item"
          ...
          <img v-if="item.online==='1'" src="~@img/yali_online.png"/>
          <img v-else-if="item.online==='2'" src="~@img/yali_outline.png"/>
          <img v-else src="~@img/yali_shilian.png"/>
...
export default {
  ...
  methods:{
    ...mapActions('watersupply/manage', ['getAllPressMacTreeData','getOnePressMacData','getPressTrendDataForOneMac']),
    // 获取所有水质监测设备
    getAllPressMac(){
      ...
      this.getAllPressMacTreeData(params).then(res=>{
        console.log('getAllPressMacTreeData',res);
    ...
    showInfo(info){
      ...
      this.detailInfoData.detailMessage.flagName = '供水压力';
      this.detailInfoData.type = 'yali';
      console.log('macId: ' + info.id, 'userId: ' + userId);
      this.getOnePressMacData({userId: userId,macId: info.id}).then(res=>{
        this.detailInfoData.detailMessage.value = res.press;
      });
      this.getPressTrendDataForOneMac({userId: userId,macId: info.id}).then(res=>{
