    ...
    <a-form :form="form" layout="horizontal" class="base-info-panel">
        ...
        <a-row>
            <a-form-item label="保障时间：" :label-col="{ span: 4 }" :wrapper-col="{ span: 18 }">
                <a-range-picker v-if="nowOptType"
                        show-time
                        v-decorator="[
                            'dayRange',
    ...
    init(){
      ...
      if(this.optType==='edit'){
        ...
        this.form.setFieldsValue({
          ...
          jobGoal: this.submitForm.jobGoal,
          jobAssignment: this.submitForm.jobAssignment,
          jobContent: this.submitForm.jobContent,
          jobRequirements: this.submitForm.jobRequirements
        });
      }
    },
    getResultData(){
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          this.submitForm.description = values.description;
          this.submitForm.jobGoal = values.jobGoal;
          this.submitForm.jobAssignment = values.jobAssignment;
          this.submitForm.jobContent = values.jobContent;
          this.submitForm.jobRequirements = values.jobRequirements;
          this.$emit('getBaseInfoData',this.submitForm)
        }
      })
