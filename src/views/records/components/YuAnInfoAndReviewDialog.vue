<template>
    <a-modal title="预案详情" v-model="yuAnInfoDialogVisible" width="1080px" class="yuan-info-dialog">
        ...
        <template slot="footer" v-if="yuAnInfo.reviewUserId==='123'">
...
<script type="text/ecmascript-6">
    ...
    export default{
      ...
      methods:{
        ...
        getYuAnDataById(){
            this.getEmergencyYuAnById({id: '1245785214'}).then((res)=>{
              this.yuAnInfo = Object.assign({},res.data);
            });
        },
        agree(){
          //调取接口修改状态（待审核->已同意）
          this.setEmergencyYuAnToPass({id: '12457532'}).then((res)=>{
            console.log(res);
        ...
        clickBack(){
          //调取接口修改状态（待审核->已驳回）
          this.setEmergencyYuAnToBack({id: '12457532',reason:this.backReason}).then((res)=>{
            console.log(res);
