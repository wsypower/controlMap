<template>
                        ...
                        <a-collapse-panel header="基本信息" key="1">
                            ...
                            <a-row>
                                <a-form-item label="预案描述：" :label-col="{ span: 4 }" :wrapper-col="{ span: 20 }">
                                    <a-textarea v-model="submitForm.description" placeholder="示例：为切实做好6月9日张学友演唱会的保障工作，确保以一流的市容环境向观看演唱会的全国各地歌迷展示金华的整体形象，根据上级的工作部署，特制定如下保障方案。" :autosize="{ minRows: 2, maxRows: 2 }" />
                                </a-form-item>
                            </a-row>
                            <a-row>
                                <a-form-item label="工作目标：" :label-col="{ span: 4 }" :wrapper-col="{ span: 20 }">
                                    <a-textarea v-model="submitForm.jobGoal" placeholder="示例：紧紧围绕建设“美丽金华”的总体目标，积极做好演唱会期间各项城市管理保障工作，确保整个城市容貌整洁、道路畅通、环境优美。" :autosize="{ minRows: 2, maxRows: 2 }" />
                                </a-form-item>
                            </a-row>
                            <a-row>
                                <a-form-item label="组织领导及任务分工：" :label-col="{ span: 4 }" :wrapper-col="{ span: 20 }">
                                    <a-textarea v-model="submitForm.jobAssignment" placeholder="示例：为加强演唱会期间保障工作的组织领导与协调，切实应对保障工作中所出现的各类问题，及时有效处置保障工作中出现的突发事件，决定成立保障工作领导小组，分局党委书记、局长陈威虎任总指挥，分局副局长余文斌任副总指挥，各中队长、科长为领导小组成员。领导小组下设专项工作组，由我分局165名人员组成，确保保障期间各项工作的顺利实施。" :autosize="{ minRows: 2, maxRows: 2 }" />
                                </a-form-item>
                            </a-row>
                            <a-row>
                                <a-form-item label="工作内容：" :label-col="{ span: 4 }" :wrapper-col="{ span: 20 }">
                                    <a-textarea v-model="submitForm.jobContent" placeholder="示例：落实蹲点劝导与机动巡查相结合的方式开展保障工作，重点做好占道经营、流动摊贩、人行道机动车与非机动车乱停放的管理，无流动无照经营、散发小广告、乱张贴等违法行为。同时配合相关部门维护现场秩序，确保人员财产安全。" :autosize="{ minRows: 2, maxRows: 2 }" />
                                </a-form-item>
                            </a-row>
                            <a-row>
                                <a-form-item label="工作要求：" :label-col="{ span: 4 }" :wrapper-col="{ span: 20 }">
                                    <a-textarea v-model="submitForm.jobRequirements" placeholder="示例：（一）统一思想、加强领导。此次保障工作时间紧，任务重，各单位要进一步提高认识，强化组织领导，认真做好前期组织动员工作，明确任务，落实责任，集中力量做好保障工作，确保保障工作不出现纰漏。
  ...
  （四）严格督查、责任追究。参加人员要统一着装（要求着短袖制服、作训帽、黑皮鞋），按时到岗，服从指挥，工作中要自觉维护队伍形象。政工科、业务督查科要加强对人员到岗、执勤工作和队容风纪等情况进行督查和通报。" :autosize="{ minRows: 2, maxRows: 2 }" />
                        ...
                        <a-collapse-panel header="人员安排" key="2">
                            <a-table :columns="columns" :dataSource="submitForm.groupData" :pagination="false">
                                ...
                                <span slot="action" slot-scope="text, record, index">
                                      <a-popconfirm v-if="submitForm.groupData.length>1" title="Sure to delete?" @confirm="() => deleteGroup(index)">
                                         <a-icon type="minus-circle" class="icon_delete"/>
                                      </a-popconfirm>
                                    <a-icon v-if="index === submitForm.groupData.length-1" type="plus-circle" class="icon_add" @click="addGroup(record,index)"/>
                                  </span>
                            </a-table>
                        </a-collapse-panel>
        ...
        <choose-people-dialog :visible.sync="choosePeopleDialogVisible"
