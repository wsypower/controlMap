    ...
    <div class="yuan_dialog_body">
      ...
      <cg-container scroll v-if="!showLoading && treeData.length > 0">
        <div class="result_item"
          ...
          <img v-if="item.online==='1'" src="~@img/bengzhan_online.png"/>
          <img v-else-if="item.online==='2'" src="~@img/bengzhan_outline.png"/>
          <img v-else src="~@img/bengzhan_shilian.png"/>
...
export default {
  ...
  mounted(){
    ...
    this.getAllDrainpumpMac();
  },
  methods:{
    ...mapActions('drainoffwater/manage', ['getAllDrainpumpMacTreeData','getOneDrainpumpMacData','getDrainpumpTrendDataForOneMac']),
    //获取市区地址范围
    // getAddressData(val){
    ...
    getAllDrainpumpMac(){
      ...
      this.getAllDrainpumpMacTreeData(params).then(res=>{
        console.log('getAllDrainpumpMacTreeData',res);
    ...
    showInfo(info){
        ...
        this.detailInfoData.detailMessage.flagName = '排水量';
        this.detailInfoData.type = 'water';
        console.log('macId: ' + info.id, 'userId: ' + userId);
        this.getOneDrainpumpMacData({userId: userId, macId:info.id}).then(res=>{
          this.detailInfoData.detailMessage.value = res.level;
          this.detailInfoData.detailMessage.levelDistance1 = res.levelDistance1;
          this.detailInfoData.detailMessage.levelDistance2 = res.levelDistance2;
          this.detailInfoData.detailMessage.momentFlow = res.momentFlow;
        });
        this.getDrainpumpTrendDataForOneMac({userId: userId, macId:info.id}).then(res=>{
