...
import { getAllPeopleTreeData, getAllPeopleDataList } from '@/api/zf/common'
export default {
  name: '',
  components: {
    PeopleInfo
  ...
  mounted() {
    ...
    _this.timer = setInterval(function() {
      getAllPeopleTreeData({ userId: userId, searchParam: _this.searchValue }).then(res => {
  ...
  methods: {
    ...
    onSearch(val) {
      ...
      const userId = util.cookies.get('userId');
      this.showLoading = true;
      getAllPeopleTreeData({ userId: userId, searchParam: this.searchValue }).then(res => {
        this.sourceData = res;
        this.showLoading = false;
      });
    },
