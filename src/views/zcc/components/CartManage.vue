<template>
  <div class="manage">
    <a-tabs v-model="activeTab" @change="changeTab" class="content_tab">
      <a-tab-pane tab="实时定位" key="1">
        <cart-position @getCarId="getCarId"></cart-position>
      </a-tab-pane>
      <a-tab-pane tab="轨迹查询" key="2">
        <cart-trail :carDataList="carDataList" :infoId="infoId"></cart-trail>
      </a-tab-pane>
      <a-tab-pane tab="违规查询" key="3">
        <violate-rules :carDataList="carDataList"></violate-rules>
      </a-tab-pane>
    </a-tabs>
  </div>
</template>
...
import CartPosition from './cartManage/CartPosition'
import CartTrail from './cartManage/CartTrail'
import ViolateRules from './cartManage/ViolateRules'
import { getAllCarDataList } from '@/api/cart/manage'
export default {
  name: 'cartManage',
  data() {
    return {
      ...
      layers.forEach(l => {
        if (l.get('featureType')) {
          if (val == '1') { //人员定位
            ...
            } else if (l.get('featureType') == 'CarRegion') {
              l.getSource().clear();
              l.setVisible(true);
            } else {
              l.setVisible(false);
