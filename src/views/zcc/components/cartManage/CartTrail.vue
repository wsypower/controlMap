    ...
    <div class="search-panel">
      <div flex="fir:left cross:center">
        ...
        <a-select v-model="query.carId" showSearch placeholder="请选择" style="width: 246px;">
          <a-select-option v-for="(car, index) in carDataList" :value="car.id" :key="index">{{ car.name }}</a-select-option>
        </a-select>
      </div>
...
import { getCarTrailDataList, getTrailDetailData } from '@/api/cart/manage'
export default {
  name: 'cartTrail',
  props: {
    infoId: {
  ...
  mounted() {
    ...
    if (this.infoId) {
      ...
      this.query.carId = this.carDataList[0].id;
    }
  ...
  methods: {
    getDataList() {
      ...
      getCarTrailDataList(this.query).then(res => {
    ...
    getCarRegion() {
      ...
      if (carData && carData.localx && carData.localx.length > 0 && carData.localy && carData.localy.length > 0) {
        ...
        if (this.regionLayer) {
          ...
          this.regionLayer.set('featureType', 'CarTrail');
        }
      }
    }
  }
